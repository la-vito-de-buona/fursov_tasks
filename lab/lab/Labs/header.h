#pragma once
#include <iostream>
#include <locale.h>
#include <Windows.h>
#include <stdio.h>
#include <string>
#include <conio.h>
#include <random>
#include <cmath>
#include <fstream>
#include <sstream>
#include <vector>
using namespace std;

#define SPACE(n) for(int i = 0; i < n; i++) cout <<" ";
#define BACK(n) for(int i = 0; i < n; i++) cout <<'\b';
#define SPACEBACK(n) SPACE(n); BACK(n);
#define ClearCin cin.clear(); cin.ignore(cin.rdbuf()->in_avail());
#define MROW 3
#define MCOL 4
#define Hi(a) (a >> (sizeof(a) * 4))
#define Lo(a) (a ^ (Hi(a) << (sizeof(a) * 4)))
#define Swap(a) (Hi(a) | (Lo(a) << (sizeof(a) * 4)))
#define LINES(a) for (int ii = 0; ii < a; ii++) cout << endl


int main();
int firstMenu();
int secondMenu();

void Info1();
void Info2();

int Lab2_1();
int Lab2_2();
int Lab3();
int Lab4();

int Lab_Maska();
int sLab_1();
int sLab_2();
int sLab_3();
int Lab_Maze();
int Lab_Types();
int EncryptMenu();

enum _eButton {
    eb_None = 0,
    eb_Left = 1,
    eb_Right = 2,
    eb_Up = 3,
    eb_Down = 4
};

float cin_cond(std::string b, float ex = INFINITE);
float maxx(float a, float b, float c, float d);
float minn(float a, float b, float c, float d);
float arr_max(float s[], const int size);
float arr_min(float s[], const int size);

void filterFile(const string & filename, bool binaryMode);
bool inputFloat(float& value, bool con);
bool inputInt(int& value, bool con);
bool inputIntf(int& value, bool con, int c);
bool inputString(string & value);
int emergencyExit();
int victory();

void SetMazeCursorPosition(int x, int y);
void SetCursorPosition(short pnRow, short pnCol);
COORD GetCursorPosition();

bool isAllOnes(double matrix[][MCOL], int rows, int col);

typedef struct _MATRIX {
    double nMatrix[MROW][MCOL]{ 0 };
    int nRow{}, nCol{};
    int InputMatrFromKb(int pindx, int pindy);
    void OutputMatrixGraf(std::string pstitle, int plndx, int pindy);
} MATRIX;



extern string shifrText;
extern string engAlphabet;
extern string rusAlphabet;
extern string numAlphabet;
extern COORD pos;

bool isValidText(const string& text);
bool inputText(string& value);


void MenuClasses();
bool ReadStringWithESC(string& out);